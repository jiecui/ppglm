...
for idxPatient = 1:length(patientList)
    ...
    for iSurr = 1:200
        disp(iSurr);
        if ~isfile([pathProcess 'surr_' num2str(iSurr) '/features/features_day_' patient '_' filterType '.csv'])
            if ~preProcessed
            ...
            % shuffle with phase randomization
            for idim = 1:size(IEADiscFilledAll, 2)
                IEADiscFilledAll(:, idim) = IAAFT(IEADiscFilledAll(:, idim))';            
            end

            % shuffle the Sz time series
            SzDiscFilledAllOrig = SzDiscFilledAll;
            SzDiscFilledAllShuffled = SzDiscFilledAll(randperm(length(SzDiscFilledAll)));
            ...
            procDatHour.SeizuresShuffled = SzDiscFilledAllShuffled;
            procDatDay.SeizuresShuffled = mean24(SzDiscFilledAllShuffled)*24;
            ...
            for hourOfDay = 0:23
                ...
                meanSzHourOfDay(hourOfDay+1) = nanmean(SzDiscFilledAllShuffled(idxHour));
                stdSzHourOfDay(hourOfDay+1) = nanstd(SzDiscFilledAllShuffled(idxHour));
            end
            ...
            for dayOfWeek = 0:6
                ...
                meanSzDayOfWeek(dayOfWeek+1, 1) = nanmean(SzDiscFilledAllShuffled(idxDay));
                stdSzDayOfWeek(dayOfWeek+1, 1) = nanstd(SzDiscFilledAllShuffled(idxDay));
            end
            ...
            if saveData
                if ~exist([pathProcess 'surr_' num2str(iSurr)], 'dir')
                    mkdir([pathProcess 'surr_' num2str(iSurr) '/'])
                end
                if ~exist([pathProcess 'surr_' num2str(iSurr) '/features'], 'dir')
                    mkdir([pathProcess 'surr_' num2str(iSurr) '/features/'])
                end

                writetable(procDatHour, [pathProcess 'surr_' num2str(iSurr) '/features/features_hour_' patient '_' filterType '.csv']);
                writetable(procDatDay, [pathProcess 'surr_' num2str(iSurr) '/features/features_day_' patient '_' filterType '.csv']);
            end
